...
function ShowUnitData()
	...
	for id, values in pairs (unitData) do
		Dprint("-------------------------------------")
		strAlive = "DEAD/NIL"
		if values.Alive then 
			strAlive = "ALIVE" 
			if values.BuilderID == 5 then
			
			Dprint("builder: " .. Locale.ToUpper(Players[ values.BuilderID ]:GetCivilizationShortDescription()) .. ", owner: " .. Locale.ToUpper(Players[ values.OwnerID ]:GetCivilizationShortDescription()) .. ", type: " .. values.Type .. ", Num. count type = " .. values.NumType)	
		...
		--Dprint(" alive = " .. strAlive .. ", combatXP = " .. values.CombatXP ..", moral = " .. values.Moral .. "/100, personnel = ".. values.Personnel .. "/".. values.MaxPersonnel ..", materiel = ".. values.Materiel .. "/".. values.MaxMateriel ..", Total = " .. totalHP .."/"..values.MaxHP)
			Dprint(" alive = " .. strAlive .. ", combatXP = " .. values.CombatXP .."")
			end
		end
	end
	Dprint("-------------------------------------")
...
function ShowResourceData()
	local bDebug = false
	Dprint("ShowResourceData ...", bDebug)
	local resourceData = MapModData.RED.ResourceData
	Dprint("Resource Data Table (loaded) :", bDebug)
	Dprint("-------------------------------------", bDebug)
	for id, values in pairs (resourceData) do
		Dprint(Players [ id ]:GetName() .. " (id=" .. id.. ")  personnel = ".. values.Personnel .. "/".. values.MaxPersonnel ..", materiel = ".. values.Materiel .. "/".. values.MaxMateriel ..", Flux = " .. values.FluxPersonnel .." pers., "..values.FluxMateriel .. " mat.", bDebug)
	end
	Dprint("-------------------------------------", bDebug)	
