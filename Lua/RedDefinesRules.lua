...
SUPPLY_FROM_CITIES		= 0.5	-- free units from cities
...
HOSPITAL_MAX_HEALING_PER_TURN		= 75	-- max number of soldiers an hospital can heal each turn
...
WAR_BONDS_PRODUCTION_PERCENT		= 100	-- percentage of production + gold converted to materiel when producing war bonds
...
PLAY_EPIC_GAME					= false -- change the calendar to a long game
...
MIN_HP_LEFT_BEFORE_ESCAPING_CITY	= 10 -- under that value of predicted HP left to a city after a combat that is starting, units will try to escape a city
...
USE_UNIT_RATIO_FOR_AI		= true	-- Allow limitation of AI military unit using ratio by classes (g_Max_Land_SubClass_Percent and g_Max_Air_SubClass_Percent must be defined in scenario files)
ALLOW_AI_UNITS_LIMIT		= false	-- Allow limitation of max number of AI military unit
MAX_AI_UNITS				= 100	-- Max number of units for the AI when ALLOW_AI_UNITS_LIMIT = true
...
AI_USE_RESOURCE_LIMIT		= false	-- the AI won't build units using resources in case of shortage...
...
DESTROYER_HUNTING_MAX_TURNS_ATTACK			= 1 -- Number of previous turns attacks entries destroyers look at for hunting
DESTROYER_HUNTING_MAX_TURNS_TO_TARGET		= 3 -- Max turns needed for destroyer to reach last attack plot for hunting
CRUISER_HUNTING_MAX_TURNS_ATTACK			= 1 -- Number of previous turns attacks entries cruisers look at for hunting
CRUISER_HUNTING_MAX_TURNS_TO_TARGET			= 3 -- Max turns needed for cruiser to reach last attack plot for hunting
BATTLESHIP_HUNTING_MAX_TURNS_ATTACK			= 1 -- Number of previous turns attacks entries battleships look at for hunting
BATTLESHIP_HUNTING_MAX_TURNS_TO_TARGET		= 3 -- Max turns needed for battleship to reach last attack plot for hunting
SUBMARINE_HUNTING_MAX_TURNS_ATTACK			= 1 -- Number of previous turns attacks entries submarines look at for hunting
SUBMARINE_HUNTING_MAX_TURNS_TO_TARGET		= 3 -- Max turns needed for submarine to reach last attack plot for hunting
...
OPTIMAL_HEALTH_PERCENT		= 50 -- try to heal until reaching this value
MIN_AIR_HEALTH_PERCENT		= 65
OPTIMAL_AIR_HEALTH_PERCENT	= 75
MIN_SEA_HEALTH_PERCENT		= 50
OPTIMAL_SEA_HEALTH_PERCENT	= 75
