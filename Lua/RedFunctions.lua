...
function PlayerBuildingRestriction(iPlayer, iBuildingType)
	...
	Dprint ("Check if player.".. iPlayer .." can construct building.".. iBuildingType, bDebug)
	...
	Dprint ("PlayerBuildingRestriction finished.", bDebug)
	return false
end
...
function PlayerTrainingRestriction(iPlayer, iUnitType)
	...
	Dprint ("PlayerTrainingRestriction finished.", bDebug)
	return false
end
...
function CachePlayerTrainingRestriction(iPlayer, iUnitType)
	...
	if UNIT_SUPPORT_LIMIT_FOR_AI and (not player:IsBarbarian()) and (not player:IsHuman()) then
		if  player:GetNumUnitsOutOfSupply() > 25 then
...
function SetMaintainCityBuilds(iPlayer)
	local player = Players[iPlayer]
	if player:IsAlive() and  (not player:IsBarbarian()) and (not player:IsHuman()) then --(not player:IsMinorCiv()) and
		for city in player:Cities() do
			...
			if unitProduction == -1 and buildingProduction == -1 and projectProduction == -1 and processProduction == -1 then
				Dprint("This city produces nothing")			
				if IsMaterielShortage(iPlayer) or IsNearMaterielShortage(iPlayer) then
					city:PushOrder (OrderTypes.ORDER_MAINTAIN, GameInfo.Processes["PROCESS_WAR_BONDS"].ID, -1, 0, false, false)
					Dprint("Push to War Bonds")			

				elseif IsPersonnelShortage(iPlayer) or IsNearPersonnelShortage(iPlayer) then
					city:PushOrder (OrderTypes.ORDER_MAINTAIN, GameInfo.Processes["PROCESS_RECRUITING"].ID, -1, 0, false, false)
					Dprint("Push to Recruiting")			
				else
					city:PushOrder (OrderTypes.ORDER_MAINTAIN, GameInfo.Processes["PROCESS_WEALTH"].ID, -1, 0, false, false)
					Dprint("Push to Wealth")			
				end
			end	
		end
	end
...
function InitializeProjects()
	...
	Dprint("-------------------------------------", bDebug)
	Dprint("Initializing new projects...", bDebug)
		...
		if startProjectInitialized ~=1 then
			for civID, projects in pairs(g_ProjectsAvailableAtStart) do
				for n, projectID in pairs(projects) do
					...
					Dprint ( Locale.ConvertTextKey(projectInfo.Description) .." is completed.", bDebug )
				end
			end
		...
		if triggered ~= 1 then -- not triggered yet, test it !
			...
			if allowedTable then -- don't check if there is no project prerequested
				for i, projectID in ipairs (allowedTable) do
					if (not IsProjectDone(projectID)) then -- not looking for a specific civs here
						Dprint ( " - " .. Locale.ConvertTextKey(projectInfo.Description) .." lacks pre-required project.", bDebug )
			...
			if ScenarioList then 
				for i, scenarioName in ipairs (ScenarioList) do
					if scenarioName == g_Scenario_Name then
						Dprint ( " - " .. Locale.ConvertTextKey(projectInfo.Description) .." available in " .. scenarioName, bDebug )
			...
			if (not bMissingProject) and (not bMissingScenario) then
				...
				if not data then -- initialize immediatly
					...
					Dprint ( " - No trigger data, " .. Locale.ConvertTextKey(projectInfo.Description) .." is immediatly available.", bDebug )
				else
					...
					if (data.Type == TRIGGER_DATE or data.Type == TRIGGER_DATE_AND_XP or data.Type == TRIGGER_DATE_OR_XP) then
						if data.Date and data.ProbPerTurn then
							...
							if data.ProbPerTurn >= rand and date.Number >= data.Date then
								...
								Dprint ( " - " .. Locale.ConvertTextKey(projectInfo.Description) .." date = Checked ! ( >= ".. tostring(data.Date) .." and Prob ".. tostring(data.ProbPerTurn) .."% >= "..tostring(rand)..")", bDebug )
							else
								bDateCheck = false
								Dprint ( " - " .. Locale.ConvertTextKey(projectInfo.Description) .." date = Fail ! ( < ".. tostring(data.Date) .." or Prob ".. tostring(data.ProbPerTurn) .."% < "..tostring(rand)..")", bDebug )
							end
						else						
							Dprint ( " - ERROR: missing data.Date or data.ProbPerTurn for " .. Locale.ConvertTextKey(projectInfo.Description) .." whith Date triggering...", bDebug )
						end
					end
						...
						if data.Reference then
							...
							if unitInfo then
								...
								if combatXP >= data.XP then
									Dprint ( " - " .. Locale.ConvertTextKey(projectInfo.Description) .." Checked ! (XP = ".. tostring(combatXP) .." >= "..tostring(data.XP)..")", bDebug )							
									...
									Dprint ( " - " .. Locale.ConvertTextKey(projectInfo.Description) .." XP = Fail ! (XP = ".. tostring(combatXP) .." < "..tostring(data.XP)..")", bDebug )
									bXPCheck = false
								end							
							else						
								Dprint ( " - ERROR: no unitInfo found  with data.Reference = " .. tostring(data.Reference) .." for " .. Locale.ConvertTextKey(projectInfo.Description), bDebug )
							end
						else						
							Dprint ( " - ERROR: no unit in data.Reference for " .. Locale.ConvertTextKey(projectInfo.Description) .." whith XP triggering...", bDebug )
						end
					end
					if	((data.Type == TRIGGER_XP or data.Type == TRIGGER_DATE) and (bDateCheck and bXPCheck)) -- if data.Type is TRIGGER_XP then bDateCheck is always true and if data.Type is TRIGGER_DATE then bXPCheck is always true...
					 ...
					 then
						...
						Dprint ( sNotificationString, bDebug )
					end
				end
			end
		end
...
function AirSweepHighlight()
		    Dprint("-------------------------------------")
		    Dprint("AirSweepHighlight is starting...")
		    Dprint("-------------------------------------")
...
function UpdatePlayerDatas(playerID)
	local bDebug = false
	local player = Players[playerID]
	if not player then
		return
	...
	Dprint("-------------------------------------")
	Dprint("Updating player #".. tostring(playerID) .." data ...", bDebug)	
	...
	Dprint("Update player datas finished", bDebug)	
...
function UpdateGlobalData()
	...
	for iPlayer = 0, GameDefines.MAX_CIV_PLAYERS - 1 do
		...
		if player and player:IsAlive() and not player:IsBarbarian() then
			UpdatePlayerDatas(iPlayer)	
			GetNumResourceTypeForPlayer(RESOURCE_OIL, iPlayer) -- update procurement detail
			Dprint("GetNumResourceTypeForPlayer-Oil: ".. GetNumResourceTypeForPlayer(RESOURCE_OIL, iPlayer),bDebug)
		end
	end	
end
