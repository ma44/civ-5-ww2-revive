...
function SetMajorRelations()
	...
	if g_Major_Diplomacy and (PreGame.GetGameOption("AlternateHistory") == 0) then

		Dprint("-------------------------------------", bDebug)
		Dprint ("Forced Diplomacy : looking for Major relationship...", bDebug)
...
function SetMinorRelations()
	...
	if g_Minor_Relation and (PreGame.GetGameOption("AlternateHistory") == 0) then

		Dprint("-------------------------------------", bDebug)
		Dprint ("Forced Diplomacy : looking for Minor relationship...", bDebug)
...
function SetMinorDOW()
	...
	if g_Major_Minor_DoW and (PreGame.GetGameOption("AlternateHistory") == 0) then

		Dprint("-------------------------------------", bDebug)
		Dprint ("Forced Diplomacy : looking for Minor civilizations Declaration of War...", bDebug)
		local turn = Game.GetGameTurn()
		-- Major vs Minor
			...
			if date <= turnDate and date > prevDate then
				if (relations) then
					for i, diploChange in ipairs (relations) do
						...
						Dprint (" - " .. player:GetName() .. " (ID "..diploChange.Major..") has signed a peace treaty with " .. player2:GetName() .. " (ID "..diploChange.Minor..")", bDebug)
					end
				end	
			end
		end
			...
			if date <= turnDate and date > prevDate then
				if (relations) then
					for i, diploChange in ipairs (relations) do
						...
						Dprint (" - " .. player:GetName() .. " (ID "..diploChange.Minor1..") has declared war against " .. player2:GetName() .. " (ID "..diploChange.Minor2..")", bDebug)
					end
				end
			end
		end
			...
			if date <= turnDate and date > prevDate then
				if (relations) then
					for i, diploChange in ipairs (relations) do
						...
						Dprint (" - " .. player:GetName() .. " (ID "..diploChange.Minor..") has declared war against " .. player2:GetName() .. " (ID "..diploChange.Major..")", bDebug)
					end
				end	
			end
		end
...
function CanSendDiploStatement(iPlayer)
	...
	if player then
		Dprint ("- DIPLO AI: Send diplo statement to " .. tostring(player:GetName()), bDebug)
		if NO_DIPLO_STATEMENT then
			return false
		end
	end
	return true
...
function InitializeClosedBorders()
	...
	Dprint("-------------------------------------", bDebug)
	Dprint("Initialize Closed Borders ", bDebug)
