...
function LoadAllTable()
	...
	local t2 = os.clock()
	Dprint("  - Total time for all tables to load:		" .. t2 - t1 .. " s")
	DEBUG_PERFORMANCE = false
end
function SaveAllTable()
	...
	local t2 = os.clock()
	Dprint("  - Total time for all tables to save :		" .. t2 - t1 .. " s")
	DEBUG_PERFORMANCE = false
end
...
function InitializeGameOption()
	...
	if USE_CUSTOM_OPTION then
		...
		if(PreGame.GetGameOption("HardcoreMode") ~= nil) and (PreGame.GetGameOption("HardcoreMode") >  0) then
			HARDCORE_MODE = true
			Dprint("- HardcoreMode	-> ON")
		end
		if(PreGame.GetGameOption("HardcoreMode") ~= nil) and (PreGame.GetGameOption("HardcoreMode") == 0) then
			HARDCORE_MODE = false
			Dprint("- HardcoreMode	-> OFF")
		end
		if(PreGame.GetGameOption("PlayEpicGame") ~= nil) and (PreGame.GetGameOption("PlayEpicGame") > 0) then
			PLAY_EPIC_GAME = true
			Dprint("- PlayEpicGame	-> ON")
		end
		if(PreGame.GetGameOption("PlayEpicGame") ~= nil) and (PreGame.GetGameOption("PlayEpicGame") == 0) then
			PLAY_EPIC_GAME = false
			Dprint("- PlayEpicGame	-> OFF")
		end
	end
end
...
function ValidateData()
	local bDebug = false
	Dprint("-------------------------------------")
	Dprint("Validating UnitClasses table...", bDebug)	
	if g_Unit_Classes then
		for class in GameInfo.UnitClasses() do
			if not g_Unit_Classes[class.ID] then
				Dprint("WARNING: ".. tostring(class.Type).." is not defined in g_Unit_Classes", bDebug)			
			end			
		end
	else
		Dprint("ERROR: g_Unit_Classes does not exist", bDebug)
	end
	
	if g_Max_Land_SubClass_Percent then	
		Dprint("-------------------------------------")
		Dprint("Validating Land SubClass Restriction table...", bDebug)
		Dprint("")
		for iPlayer = 0, GameDefines.MAX_CIV_PLAYERS - 1 do
			local player = Players[iPlayer]
			if player:IsEverAlive() and not player:IsMinorCiv() and not player:IsBarbarian() then
				Dprint("- Checking for ".. tostring(player:GetName()) .."...", bDebug)
				local civID = GetCivIDFromPlayerID(iPlayer)
				if g_Max_Land_SubClass_Percent[civID] then
					local totalPercent = 0
					for iClass, percent in pairs(g_Max_Land_SubClass_Percent[civID]) do
						...
						if checkClass then
							Dprint(" - Found unit for " .. Locale.ConvertTextKey(GameInfo.UnitClasses[iClass].Description) .." (" .. tostring(classPercent) .."% allowed)", bDebug)
						end
					end
					if totalPercent <= 100 then					
						Dprint("  - Ok ! (total = ".. totalPercent .."%)", bDebug)
					else
						Dprint("WARNING: Validating Land SubClass Restriction failed for ".. tostring(player:GetName()) .." (total = ".. tostring(totalPercent) .."%)", bDebug)
					end
				end
				Dprint("")
			end
	...
	if g_Max_Air_SubClass_Percent then	
		Dprint("-------------------------------------")
		Dprint("Validating Air SubClass Restriction table...", bDebug)
		Dprint("", bDebug)
		for iPlayer = 0, GameDefines.MAX_CIV_PLAYERS - 1 do
			local player = Players[iPlayer]
			if player:IsEverAlive() and not player:IsMinorCiv() and not player:IsBarbarian() then
				...
				if g_Max_Air_SubClass_Percent[civID] then
					...
					if totalPercent <= 100 then					
						...
						Dprint("WARNING: Validating Air SubClass Restriction Failed ! for ".. tostring(player:GetName()) .." (total = ".. tostring(totalPercent) .."%)", bDebug)
					end
				end
				Dprint("", bDebug)
			end
	...
	if g_Max_Sea_SubClass_Percent then	
		Dprint("-------------------------------------")
		Dprint("Validating Sea SubClass Restriction table...", bDebug)
		Dprint("", bDebug)
		for iPlayer = 0, GameDefines.MAX_CIV_PLAYERS - 1 do
			local player = Players[iPlayer]
			if player:IsEverAlive() and not player:IsMinorCiv() and not player:IsBarbarian() then
				Dprint("- Checking for ".. tostring(player:GetName()) .."...", bDebug)
				local civID = GetCivIDFromPlayerID(iPlayer)
				if g_Max_Sea_SubClass_Percent[civID] then
					local totalPercent = 0
					for iClass, percent in pairs(g_Max_Sea_SubClass_Percent[civID]) do
						local checkClass = false
						local classPercent = 0
						for unitInfo in GameInfo.Units() do
							local iUnitType = unitInfo.ID
					...
					if totalPercent <= 100 then					
						...
						Dprint("WARNING: Validating Sea SubClass Restriction Failed ! for ".. tostring(player:GetName()) .." (total = ".. tostring(totalPercent) .."%)", bDebug)
					end
				end
				Dprint("", bDebug)
			end
...
function FinalizeNextPlayerProjects(CurrentPlayerID)
	local bDebug = false
	Dprint ("-------------------------------------", bDebug)
	Dprint ("Finalize projects of next player...", bDebug)
	local bDebug = false
...
function GetSupplyFreeUnits(player)
	...
	for unit in player:Units() do
		if (unit:GetUnitType() == CONVOY) or (unit:GetUnitType() == FORTIFIED_GUN) or (unit:GetUnitType() == FR_PARTISAN)then
