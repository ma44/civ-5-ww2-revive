...
function CountUnitType (unitType)
	local bDebug = false
	local num = 0
	Dprint ("unitType: "..unitType, bDebug)
	for id, values in pairs (MapModData.RED.UnitData) do
...
function IsAAGun(iNumType)
	return (    iNumType == CLASS_AA_GUN
			 or iNumType == CLASS_MOBILE_AA_GUN
			 or iNumType == CLASS_MOBILE_AA_GUN_B
			 or iNumType == CLASS_MOBILE_AA_GUN_2
			 or iNumType == CLASS_MOBILE_AA_GUN_2_B
			 or iNumType == CLASS_MOBILE_AA_GUN_2_C
			 or iNumType == CLASS_MOBILE_AA_GUN_2_D
			 or iNumType == CLASS_MOBILE_AA_GUN_3
			)
end
...
function IsCarrierClass(iNumType)
	return (    iNumType == CLASS_CARRIER
			 or iNumType == CLASS_CARRIER_2
			 or iNumType == CLASS_CARRIER_3
			)
end

function IsCarrierPlane(unitClass)
	return (    unitClass == CLASS_OLD_NAVY_FIGHTER
			 or unitClass == CLASS_NAVY_FIGHTER
			 or unitClass == CLASS_NAVY_FIGHTER_B
			 or unitClass == CLASS_NAVY_FIGHTER_C
			 or unitClass == CLASS_NAVY_FIGHTER_2
			 or unitClass == CLASS_NAVY_FIGHTER_3
			 or unitClass == CLASS_NAVY_BOMBER
			 or unitClass == CLASS_NAVY_BOMBER_2
			 or unitClass == CLASS_NAVY_LIGHT_BOMBER
			 or unitClass == CLASS_TORPEDO_BOMBER 
			)
end
...
function CacheIsLimitedByRatio(unitType, playerID, civID, totalUnits, numDomain, bDebug )
	...
	local totalUnits = player:GetNumUnitsSupplied()
	...
	if GameInfo.Units[unitType].Domain == "DOMAIN_AIR" then
		...
		if g_Max_Air_SubClass_Percent and g_Max_Air_SubClass_Percent[civID] then
			local totalUnitsPossible = (totalUnits/g_Combat_Type_Ratio[civID].Air)
			local airType = g_Unit_Classes[unitClass].NumType
			local maxPercent = g_Max_Air_SubClass_Percent[civID][airType]
			if maxPercent and (totalUnitsPossible > 0) and ((aliveUnitSubClass / totalUnitsPossible) * 100 > maxPercent) then
				g_UnitRestrictionString = "Air type restriction (aliveUnitSubClass / totalUnitsPossible * 100 > g_Max_Air_SubClass_Percent[civID][airType]) -> (".. aliveUnitSubClass .." / ".. totalUnitsPossible .." * 100 > ".. maxPercent..")"
				return true
			end
		end
	...
	elseif GameInfo.Units[unitType].Domain == "DOMAIN_SEA" then
		...
		if g_Max_Sea_SubClass_Percent and g_Max_Sea_SubClass_Percent[civID] then
			local totalUnitsPossible = (totalUnits/g_Combat_Type_Ratio[civID].Sea)
			local seaType = g_Unit_Classes[unitClass].NumType
			local maxPercent = g_Max_Sea_SubClass_Percent[civID][seaType]
			if maxPercent and (totalUnitsPossible > 0) and (aliveUnitSubClass / totalUnitsPossible * 100 > maxPercent) then
				g_UnitRestrictionString = "Sea type restriction (aliveUnitSubClass / totalUnitsPossible * 100 > g_Max_Sea_SubClass_Percent[civID][seaType]) -> (".. aliveUnitSubClass .." / ".. totalUnitsPossible .." * 100 > ".. maxPercent..")"
				return true
			end
		end
		...
		if g_Combat_Type_Ratio and g_Combat_Type_Ratio[civID] and g_Combat_Type_Ratio[civID].Land then
			if (numDomain > 0) and (totalUnits/numDomain < g_Combat_Type_Ratio[civID].Land) then
				g_UnitRestrictionString = "Land ratio restriction (totalUnits/numDomain < g_Combat_Type_Ratio[civID].Land) -> (".. totalUnits .." / ".. numDomain .." < ".. g_Combat_Type_Ratio[civID].Land..")"
				return true
			end
		end		
		if g_Max_Land_SubClass_Percent and g_Max_Land_SubClass_Percent[civID] then
			local totalUnitsPossible = (totalUnits/g_Combat_Type_Ratio[civID].Land)
			local landType = g_Unit_Classes[unitClass].NumType
			local maxPercent = g_Max_Land_SubClass_Percent[civID][landType]
			if maxPercent and (totalUnitsPossible > 0) and (aliveUnitSubClass / totalUnitsPossible * 100 > maxPercent) then
				g_UnitRestrictionString = "Land type restriction (aliveUnitSubClass / totalUnitsPossible * 100 > g_Max_Land_SubClass_Percent[civID][landType]) -> (".. aliveUnitSubClass .." / ".. totalUnitsPossible .." * 100 > ".. maxPercent..")"
				return true
			end
		end
	end
