...
function CombatResult (iAttackingPlayer, iAttackingUnit, attackerDamage, attackerFinalDamage, attackerMaxHP, iDefendingPlayer, iDefendingUnit, defenderDamage, defenderFinalDamage, defenderMaxHP, iInterceptingPlayer, iInterceptingUnit, interceptorDamage, plotX, plotY)
	...
	if pAttackingPlayer then -- In case the combat was aborted...
		...
		elseif pDefendingCity then
			...
			if unitCount > 0 then 
				Dprint ("List of units in the city : ", g_DebugCombat)
			end
			for i = 0, unitCount - 1, 1 do	
    			local testUnit = pDefendedPlot:GetUnit(i)
				if testUnit then
					Dprint("   - unit num" .. i+1 .. " : " .. testUnit:GetName(), g_DebugCombat)
				end
			end
			if bEscapeCity then
		...
		elseif pDefendingUnit and pDefendingUnit:IsHasPromotion( GameInfo.UnitPromotions.PROMOTION_CONVOY.ID ) then
			Dprint ("Attack type : Hunting Convoy", g_DebugCombat)
			combatType = CONHUNT

		elseif pDefendingUnit and (pDefendingUnit:IsHasPromotion( GameInfo.UnitPromotions.PROMOTION_EMBARKED_FIX.ID) or pDefendingUnit:IsHasPromotion( GameInfo.UnitPromotions.PROMOTION_EMBARKED_ARTILLERY_FIX.ID)) then
			Dprint ("Attack type : Hunting Embarked Unit", g_DebugCombat)
			combatType = EMBHUNT

		elseif pAttackingUnit:IsHasPromotion( GameInfo.UnitPromotions.PROMOTION_DESTROYER.ID ) then
			Dprint ("Attack type : From Destroyer", g_DebugCombat)
			combatType = DESATTACK

		elseif pDefendingUnit and pDefendingUnit:IsHasPromotion( GameInfo.UnitPromotions.PROMOTION_DESTROYER.ID ) then
			Dprint ("Attack type : Hunting Submarine", g_DebugCombat)
			combatType = DESHUNT

		elseif pAttackingUnit:IsHasPromotion( GameInfo.UnitPromotions.PROMOTION_CRUISER.ID ) then
			Dprint ("Attack type : From Destroyer", g_DebugCombat)
			combatType = CRUATTACK

		elseif pDefendingUnit and pDefendingUnit:IsHasPromotion( GameInfo.UnitPromotions.PROMOTION_CRUISER.ID ) then
			Dprint ("Attack type : Hunting Submarine", g_DebugCombat)
			combatType = CRUHUNT

		elseif pAttackingUnit:IsHasPromotion( GameInfo.UnitPromotions.PROMOTION_BATTLESHIP.ID ) then
			Dprint ("Attack type : From Destroyer", g_DebugCombat)
			combatType = BATATTACK

		elseif pDefendingUnit and pDefendingUnit:IsHasPromotion( GameInfo.UnitPromotions.PROMOTION_BATTLESHIP.ID ) then
			Dprint ("Attack type : Hunting Submarine", g_DebugCombat)
			combatType = BATHUNT
		...
		if not pAttackingUnit:IsRanged() then
			...
			if diffDamage > 0 then				
				...
				if (ratioHitPoint < RETREAT_RATIO or defenderDamage >= (MAX_HP / RETREAT_RATIO)) and defenderHealth > 0 then -- Max received damage is now hardcoded in the DLL to be GameDefines.MAX_HIT_POINTS / 5, but check for >= anyway.		
...
function CombatEnded (iAttackingPlayer, iAttackingUnit, attackerDamage, attackerFinalDamage, attackerMaxHP, iDefendingPlayer, iDefendingUnit, defenderDamage, defenderFinalDamage, defenderMaxHP, iInterceptingPlayer, iInterceptingUnit, interceptorDamage, plotX, plotY)
	...
	if pAttackingPlayer then -- In case the combat was aborted...
		...
		elseif pDefendingUnit and (pDefendingUnit:IsHasPromotion( GameInfo.UnitPromotions.PROMOTION_EMBARKATION.ID)) then
			Dprint ("Attack type : Hunting Embarked Unit", g_DebugCombat)
			combatType = EMBHUNT
		...
		elseif pDefendingUnit and pDefendingUnit:IsHasPromotion( GameInfo.UnitPromotions.PROMOTION_DESTROYER.ID ) then
			Dprint ("Attack type : Hunting Destroyer", g_DebugCombat)
			combatType = DESHUNT

		elseif pAttackingUnit:IsHasPromotion( GameInfo.UnitPromotions.PROMOTION_CRUISER.ID ) then
			Dprint ("Attack type : From Cruiser", g_DebugCombat)
			combatType = CRUATTACK

		elseif pDefendingUnit and pDefendingUnit:IsHasPromotion( GameInfo.UnitPromotions.PROMOTION_CRUISER.ID ) then
			Dprint ("Attack type : Hunting Cruiser", g_DebugCombat)
			combatType = CRUHUNT

		elseif pAttackingUnit:IsHasPromotion( GameInfo.UnitPromotions.PROMOTION_BATTLESHIP.ID ) then
			Dprint ("Attack type : From Battleship", g_DebugCombat)
			combatType = BATATTACK

		elseif pDefendingUnit and pDefendingUnit:IsHasPromotion( GameInfo.UnitPromotions.PROMOTION_BATTLESHIP.ID ) then
			Dprint ("Attack type : Hunting Battleship", g_DebugCombat)
			combatType = BATHUNT

		elseif pDefendingUnit and pDefendingUnit:IsHasPromotion( GameInfo.UnitPromotions.PROMOTION_CARRIER.ID ) then
			Dprint ("Attack type : Hunting Carrier", g_DebugCombat)
			combatType = CARHUNT
...
function TacticalAILaunchUnitAttack(iPlayer, iUnit, x, y)
	...
	if unit then
		Dprint ("TacticalAILaunchUnitAttack for " .. tostring(unit:GetName()) .. " of player " .. tostring(player:GetName()) .. " at " .. tostring(x) .. "," .. tostring(y), g_DebugCombat)
...
function PushingMissionTo(iPlayer, iUnit, x, y, iMission)
	...
	local unit = player:GetUnitByID(iUnit)
	if unit then
		Dprint ("Pushing Mission (type=".. iMission ..") for " .. tostring(unit:GetName()) .. " of player " .. tostring(player:GetName()) .. " to " .. tostring(x) .. "," .. tostring(y), g_DebugCombat)
...
function FirstStrike(iPlayer, iUnit, x, y, iMission)
	...
	Dprint ("Check for First Strike before attack of " .. tostring(attackingUnit:GetName()) .. " of player " .. tostring(player:GetName()) .. " against " .. tostring(x) .. "," .. tostring(y), g_DebugCombat)
