...
function GetCityMateriel(city, bUpdate)
	...
	cityMateriel.fromProdYield, cityMateriel.fromFactory, cityMateriel.fromHarbor, cityMateriel.fromWarBonds,  cityMateriel.fromTrait = 0, 0, 0, 0, 0
	...
	-- Apply bonuses
	if not bCaptured and GetCivIDFromPlayerID (city:GetOwner()) == GERMANY then cityMateriel.fromTrait = (cityMateriel.fromProdYield + cityMateriel.fromFactory + cityMateriel.fromHarbor + cityMateriel.fromWarBonds) * (100) / 100; end -- German trait

	-- Total
	cityMateriel.total = cityMateriel.total + cityMateriel.fromTrait
...
function GetCityMaterielTooltip(cityMateriel)
	...
	if (cityMateriel.fromTrait ~= 0) then
		strMaterielToolTip = strMaterielToolTip .. "[ICON_BULLET]" .. Locale.ConvertTextKey("TXT_KEY_MATERIEL_FROM_TRAIT", cityMateriel.fromTrait);
		strMaterielToolTip = strMaterielToolTip .. "[NEWLINE]";
	end


	local iBase = cityMateriel.fromProdYield + cityMateriel.fromFactory + cityMateriel.fromHarbor + cityMateriel.fromWarBonds + cityMateriel.fromTrait
...
function GetMaterielReinforcement (playerID, bUpdate)
	...
	materielReinforcement.fromGlobal, materielReinforcement.fromFactory, materielReinforcement.fromHarbor, materielReinforcement.fromWarBonds, materielReinforcement.fromMinor, materielReinforcement.fromScenario, materielReinforcement.fromTrait  = 0, 0, 0, 0, 0, 0, 0
	...
	for city in player:Cities() do
		...
		materielReinforcement.fromTrait		= materielReinforcement.fromTrait + cityMateriel.fromTrait
	...
	materielReinforcement.total = materielReinforcement.fromGlobal + materielReinforcement.fromFactory + materielReinforcement.fromHarbor + materielReinforcement.fromWarBonds + materielReinforcement.fromMinor + materielReinforcement.fromScenario + materielReinforcement.fromTrait
...
function GetCityMaxMateriel (city)
	...
	if city:IsHasBuilding(SMALL_AIR_FACTORY) then max = max + 25; end
	if city:IsHasBuilding(MEDIUM_AIR_FACTORY) then max = max + 25; end
	if city:IsHasBuilding(LARGE_AIR_FACTORY) then max = max + 25; end
...
function GetOilProcurement (playerID, bNoUpdate)
	...
	oilProcurement.fromGlobal, oilProcurement.fromScenario, oilProcurement.fromMap, oilProcurement.fromBuildings, oilProcurement.fromTrait = 0, 0, 0, 0, 0
	...
	local team = Teams[ pAmerica:GetTeam() ]
	if team:IsAtWar( pGermany:GetTeam() ) or team:IsAtWar( pJapan:GetTeam() )then
		if iplayerID == GetPlayerIDFromCivID (AMERICA, false, true) then 
			oilProcurement.fromTrait = (oilProcurement.fromGlobal + oilProcurement.fromMap + oilProcurement.fromBuildings) * (100) / 100;  -- US trait
		end
	end

		-- Total OIL

	oilProcurement.total = oilProcurement.fromGlobal + oilProcurement.fromScenario + oilProcurement.fromMap + oilProcurement.fromBuildings + oilProcurement.fromTrait
...
function InitializeResourceTable()
	...
	local bDebug = false
	Dprint("-------------------------------------", bDebug)
	Dprint("Creating Resource data table...")
	...
	for playerID = 0, GameDefines.MAX_CIV_PLAYERS - 1 do
		...
		if (player:IsAlive()) then
			...
			resourceData[playerID].OilFromTrait				= oilProcurement.fromTrait
			...
			Dprint("- " .. player:GetName() .. " | +" .. fluxPersonnel .."/"..maxPersonnel.." personnel | +".. fluxMateriel .."/"..maxMateriel.." materiel | +".. fluxOil .."/"..maxOil.." oil", bDebug)
...
function ManageResources(playerID) 
	...
	if ( player:IsAlive() ) then
		...
		MapModData.RED.ResourceData[playerID].OilFromTrait		= oilProcurement.fromTrait
...
function InitializeResourceMap() 
	local bDebug = false
	Dprint("-------------------------------------", bDebug)
	Dprint("Initializing resource map ...", bDebug)		
	local t1 = os.clock()
	...
	local t2 = os.clock()
	Dprint("  - Total time to initialize resource map :		" .. t2 - t1 .. " s", bDebug)
	Dprint("-------------------------------------", bDebug)
end
...
function GetNumResourceTypeForPlayer(resourceID, playerID)
	...
	Dprint("-------------------------------------", bDebug)
	Dprint("Getting Number of resource type ID=".. tostring(resourceID) .." available for player ID=" .. tostring(playerID), bDebug)
	local numResource = 0
	...
	for plotKey, data in pairs(MapModData.RED.ResourceMap) do
		if MapModData.RED.ResourceMap[plotKey].Type == resourceID then
			...
			if plot:IsImprovementPillaged() then
				...
				Dprint("- Improvement pillaged at " .. plotKey, bDebug)
			else
				local ownerID = plot:GetOwner()
				local player = Players[playerID]
				Dprint("- Available resource found at " .. plotKey .. " check for relation with owner (id =" .. tostring(ownerID) ..")", bDebug)
				...
				if not RESOURCE_FROM_FRIENDS and ownerID ~= playerID then
					...
					Dprint("  - Can't access resource (not in player territory and no access to friend resources from scenario setting)", bDebug)
				elseif player:IsMinorCiv() and ownerID ~= playerID then
					-- Minor civ only access their resources
					Dprint("  - Can't access resource (not in this minor civ territory)", bDebug)
				else
					...
					if bOnFriendlyTerritory then
						Dprint("  - Resource is on friendly territory, check for possible route", bDebug)

						local bCanGetRessource = false

						if RESOURCE_CONNECTION == RESOURCE_OWNED_PLOTS then
							Dprint("   - No connection required from scenario settings...", bDebug)
						...
						elseif	RESOURCE_CONNECTION == RESOURCE_ROAD_TO_CAPITAL then
							...
							Dprint("   - Looking for route to capital...", bDebug)
						...
						elseif	RESOURCE_CONNECTION == RESOURCE_RAILS_TO_CAPITAL then
							...
							Dprint("   - Looking for rails to capital...", bDebug)
							local capital = player:GetCapitalCity()
							if capital then
						...
						elseif	RESOURCE_CONNECTION == RESOURCE_ROAD_TO_ANY_CITY then
							...
							Dprint("   - Looking for route to any city...", bDebug)
						...
						elseif	RESOURCE_CONNECTION == RESOURCE_RAILS_TO_ANY_CITY then
							...
							Dprint("   - Looking for rails to any city...", bDebug)
							-- first check closest own cities
							local closeCity = GetCloseCity ( playerID, plot )
							if closeCity then
								local cityPlot = closeCity:Plot()
						...
						if bCanGetRessource then
							Dprint("   - Resource is connected, added to pool...", bDebug)
