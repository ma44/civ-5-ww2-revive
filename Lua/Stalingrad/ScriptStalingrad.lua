...
function SetStalingradPromotion( playerID, unitID)
	...
	if (GetCivIDFromPlayerID (playerID) == USSR and unit:GetDomainType() == DomainTypes.DOMAIN_LAND) then
		if (not unit:IsHasPromotion(PROMOTION_ARCTIC_POWER)) then
			Dprint("   - Adding Arctic Power promotion to " .. unit:GetName() .. " (unitID =".. unit:GetID(), bDebug)
...
function UpdateStalingradScore()
	local bDebug = false
	Dprint("-------------------------------------", bDebug)
	Dprint("Updating Stalingrad Score...", bDebug)
...
function CheckStalingradVictory()
	...
	if Game.GetGameTurn() > STALINGRAD_END_TURN then
		...
		elseif (score[GERMANY] > score[USSR]) and GetCivIDFromPlayerID (stalingradOwner) == GERMANY then
			Dprint("Winner is GERMANY !", bDebug)
			winner = GERMANY
			scoreBonus = (score[GERMANY] - score[USSR])*5
		end
		if not winner then 		
			Dprint("No winner yet, entering Extra time...", bDebug)
		...
		Dprint("Bonus from Stalingrad control is " .. scoreBonus, bDebug)
		...
		if GetCivIDFromPlayerID (Game:GetActivePlayer()) == winner then
			Dprint("Active player is the Winner !", bDebug)
			...
			if winner == GERMANY then -- Winner is Germany
				...
				Dprint("6th Army score is " .. sixthArmyScore, bDebug)
				...
				Dprint("Rail score is " .. railScore, bDebug)
				...
				Dprint("Road score is " .. roadScore, bDebug)
...
function CreateTerritoryMap()
	local bDebug = false
	Dprint("-------------------------------------", bDebug)
	Dprint("Creating Territory Map...", bDebug)
