...
function PopulateDiplomatic()
	...
	if(PreGame.IsVictory(GameInfo.Victories["VICTORY_DIPLOMATIC"].ID))then
		...
		if(UNHome ~= nil) then
			...
			elseif(pPlayer:GetID() == Game.GetActivePlayer()) then
				if(PreGame.GetCivilizationShortDescription(pPlayer:GetID()) ~= "") then
					...
					strPlayer = Locale.Lookup(myCivInfo.ShortDescription);
				end
			else
				strPlayer = Locale.Lookup(myCivInfo.ShortDescription);
			end
			...
			Controls.UNInfo:LocalizeAndSetText("TXT_KEY_VP_DIPLO_PROJECT_BUILT_BY", strPlayer, "TXT_KEY_BUILDING_UNITED_NATIONS");
		else
			...
			Controls.UNInfo:LocalizeAndSetText("TXT_KEY_VP_DIPLO_PROJECT_BUILT_BY", "TXT_KEY_CITY_STATE_NOBODY", "TXT_KEY_BUILDING_UNITED_NATIONS");
		end
		...
		Controls.VotesHave:SetText(Teams[curTeam]:GetTotalSecuredVotes());
	...
	if(PreGame.IsVictory(GameInfo.Victories["VICTORY_CULTURAL"].ID))then
		...
		if (Teams[leadAI:GetTeam()]:IsHasMet(Game.GetActiveTeam()) or Game:IsNetworkMultiPlayer()) then
			if(leadAI:GetNickName() ~= "" and Game:IsNetworkMultiPlayer()) then
				...
				strCiv = Locale.Lookup(myCivInfo.ShortDescription);
			end
		else
			strCiv = Locale.Lookup("TXT_KEY_MISC_UNKNOWN");
		end
