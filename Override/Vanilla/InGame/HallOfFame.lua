	...
	for i, v in pairs(games) do
		...
		if(civ ~= nil) then
			...
			if(leader ~= nil) then
		        ...
		        IconHookup(4, 32, "WORLDTYPE_ATLAS", controlTable.MapType);
		        controlTable.MapType:SetToolTipString("");
				if(mapScript ~= nil) then
					IconHookup( mapScript.IconIndex or 0, 32, mapScript.IconAtlas, controlTable.MapType );        
					controlTable.MapType:SetToolTipString( Locale.ConvertTextKey( mapScript.Name ) );
				end
