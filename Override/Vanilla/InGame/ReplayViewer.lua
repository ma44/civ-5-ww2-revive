...
Panels = {
	...
	
	{
		...
		DrawGraph = function(panel)
			...
			local xIncrements = {
				0, .025, .05, .075, .1, .125, .15, .175, .2, .25,
				.275, .3, .325, .35, .375, .4, .425, .45, .475, .5,
				.525, .55, .575, .6, .625, .65, .675, .7, .725, .75,
				.775, .8, .825, .850, .875, .9, .925, .95, .975, 1.0
			};
			...
			function DrawGraph(playerInfo, color, scoreType, graphWidth, minX, maxX, yScale, yOffset)
				...
				for i, v in pairs(playerInfo.Scores) do
					...
					if(value ~= nil and i <= maxX and i >= minX) then
						table.insert(turnData, {i - minX, value});					
					end
				end
				...
				-- Ensure data is sorted by turns.
				table.sort(turnData, function(a, b) return a[1] < b[1]; end);
				
				-- Second, determine the number of lines to be drawn.
				-- A new line is drawn if there is a hole in the turn data.
				local lines = {};
				local lineStart = 1;
				local prevY;
				for i, v in ipairs(turnData) do
					if(prevY ~= nil and prevY ~= v[1] - 1) then
						table.insert(lines, {lineStart, i - 1});
						lineStart = i;
					end
					
					prevY = v[1];
				end
				table.insert(lines, {lineStart, #turnData});
				
				
				function GetValueAtTurn(turn)
					for i,v in ipairs(turnData) do
						if(v[1] == turn) then
							return v[2];
						end
				...
				-- Now iterate through each line and draw the segments.
				-- NOTE: We can have roughly 5,150 segments in total for 
				-- this graph which means we must limit the number of 
				-- segments per line!
				-- Let's only draw a segment per marker.
				for _ , line in ipairs(lines) do
				
					local lineStart = turnData[line[1]];
					local lineStartTurn = lineStart[1];
					
					local lineEnd = turnData[line[2]];
					local lineEndTurn = lineEnd[1];
					
					local startX = lineStartTurn * xScale;
					local startY = math.floor(lineStart[2] - yOffset) * yScale;
					
					startY = graphHeight - startY;
					
					local lineSegment = CreateLineSegment(startX, startY);
					
					for _, xIncrement in ipairs(xIncrements) do
					
						local turn = (xIncrement * (maxX - minX)) + minX;
						local beforeTurn = math.floor(turn);
						local afterTurn = math.ceil(turn);
					
						if(beforeTurn >= lineStartTurn) then
							if(afterTurn <= lineEndTurn) then 
								
								local valueBeforeTurn = GetValueAtTurn(beforeTurn);
								local valueAfterTurn = GetValueAtTurn(afterTurn);
								
								local lerpValue = lerp(valueBeforeTurn, valueAfterTurn, turn - beforeTurn) ;
								
								local newX = math.floor(turn * xScale);
								local newY = math.floor((lerpValue - yOffset) * yScale);
								
								--inverse Y
								newY = graphHeight - newY;	
								
								lineSegment.LineSegment:SetEndVal(newX, newY);
								lineSegment = CreateLineSegment(newX, newY);
							end	
					...
					local endX = math.floor(lineEndTurn * xScale);
					local endY = math.floor((lineEnd[2] - yOffset) * yScale);		
