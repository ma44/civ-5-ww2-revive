...
-- China	
	sortedList[CategoryUnits][1] = {};
	local tableid = 1;
	for unit in GameInfo.Units() do
		if unit.Requirements == 'CHINA' then
			if unit.ShowInPedia then
				...
				sortedList[CategoryUnits][1][tableid] = article;
				tableid = tableid + 1;
...
-- France	
	sortedList[CategoryUnits][2] = {};
	local tableid = 1;
	for unit in GameInfo.Units() do
		if unit.Requirements == 'FRANCE' then
			if unit.ShowInPedia then
				local article = {};
				local name = Locale.ConvertTextKey( unit.Description )
				article.entryName = name;
				...
				sortedList[CategoryUnits][2][tableid] = article;
				tableid = tableid + 1;
...
-- Germany
	sortedList[CategoryUnits][3] = {};
	local tableid = 1;
	for unit in GameInfo.Units() do
		if unit.Requirements == 'GERMANY' then
			if unit.ShowInPedia then
				local article = {};
				local name = Locale.ConvertTextKey( unit.Description )
				article.entryName = name;
				...
				sortedList[CategoryUnits][3][tableid] = article;
				tableid = tableid + 1;
...
-- Greece
	sortedList[CategoryUnits][4] = {};
	local tableid = 1;
	for unit in GameInfo.Units() do
		if unit.Requirements == 'GREECE' then
			if unit.ShowInPedia then
				local article = {};
				local name = Locale.ConvertTextKey( unit.Description )
				article.entryName = name;
				...
				sortedList[CategoryUnits][4][tableid] = article;
				tableid = tableid + 1;
...
-- Italy
	sortedList[CategoryUnits][5] = {};
	local tableid = 1;
	for unit in GameInfo.Units() do
		if unit.Requirements == 'ROME' then
			if unit.ShowInPedia then
				local article = {};
				local name = Locale.ConvertTextKey( unit.Description )
				article.entryName = name;
				...
				sortedList[CategoryUnits][5][tableid] = article;
				tableid = tableid + 1;
...
-- Japan
	sortedList[CategoryUnits][6] = {};
	local tableid = 1;
	for unit in GameInfo.Units() do
		if unit.Requirements == 'JAPAN' then
			if unit.ShowInPedia then
				local article = {};
				local name = Locale.ConvertTextKey( unit.Description )
				article.entryName = name;
				...
				sortedList[CategoryUnits][6][tableid] = article;
				tableid = tableid + 1;
...
-- Poland
	sortedList[CategoryUnits][7] = {};
	local tableid = 1;
	for unit in GameInfo.Units() do
		if unit.Requirements == 'POLAND' then
			if unit.ShowInPedia then
				local article = {};
				local name = Locale.ConvertTextKey( unit.Description )
				article.entryName = name;
				...
				sortedList[CategoryUnits][7][tableid] = article;
				tableid = tableid + 1;
...
-- Russia
	sortedList[CategoryUnits][8] = {};
	local tableid = 1;
	for unit in GameInfo.Units() do
		if unit.Requirements == 'RUSSIA' then
			if unit.ShowInPedia then
				local article = {};
				local name = Locale.ConvertTextKey( unit.Description )
				article.entryName = name;
				...
				sortedList[CategoryUnits][8][tableid] = article;
				tableid = tableid + 1;
...
-- UK
	sortedList[CategoryUnits][9] = {};
	local tableid = 1;
	for unit in GameInfo.Units() do
		if unit.Requirements == 'ENGLAND' then
			if unit.ShowInPedia then
				local article = {};
				local name = Locale.ConvertTextKey( unit.Description )
				article.entryName = name;
				...
				sortedList[CategoryUnits][9][tableid] = article;
				tableid = tableid + 1;
...
-- USA
	sortedList[CategoryUnits][10] = {};
	local tableid = 1;
	for unit in GameInfo.Units() do
		if unit.Requirements == 'AMERICA' then
			if unit.ShowInPedia then
				local article = {};
				local name = Locale.ConvertTextKey( unit.Description )
				article.entryName = name;
				...
				sortedList[CategoryUnits][10][tableid] = article;
				tableid = tableid + 1;
	...
	-- Other nations
	sortedList[CategoryUnits][11] = {};
	local tableid = 1;
	for unit in GameInfo.Units() do
		if unit.Requirements == 'OTHER' then
			if unit.ShowInPedia then
				local article = {};
				local name = Locale.ConvertTextKey( unit.Description )
				article.entryName = name;
				...
				sortedList[CategoryUnits][11][tableid] = article;
				tableid = tableid + 1;
...
CivilopediaCategory[CategoryUnits].SelectHeading = function( selectedSection, dummy )
	print("CivilopediaCategory[CategoryUnits].SelectHeading");
	g_ListHeadingManager:ResetInstances();
	g_ListItemManager:ResetInstances();

	sortedList[CategoryUnits][selectedSection].headingOpen = not sortedList[CategoryUnits][selectedSection].headingOpen; -- ain't lua great
	...
	-- add a section header
		...
		if thisHeaderInstance then
			...
			thisHeaderInstance.ListHeadingButton:SetVoids( section, 0 );
			thisHeaderInstance.ListHeadingButton:RegisterCallback( Mouse.eLClick, CivilopediaCategory[CategoryUnits].SelectHeading );
			otherSortedList[tostring( thisHeaderInstance.ListHeadingButton )] = sortOrder;
		end	
...
CivilopediaCategory[CategoryUnits].DisplayList = function()
	...
	for section = 1,11,1 do
		...
		local thisHeaderInstance = g_ListHeadingManager:GetInstance();
		if thisHeaderInstance then
			sortedList[CategoryUnits][section].headingOpen = false; -- ain't lua great
			sortOrder = sortOrder + 1;
			local textString = "TXT_KEY_UNITS_SECTION_"..tostring( section );
			local localizedLabel = "[ICON_MINUS] "..Locale.ConvertTextKey( textString );
			thisHeaderInstance.ListHeadingLabel:SetText( localizedLabel );
