...
-------------------------------------------------
-- If we hear a MultiplayerGameInvite was sent, exit
...
function ShowHideHandler( bIsHide, bIsInit )
    ...
    if( not bIsHide ) then
		...
		Controls.SPQuickCombatCheckBox:SetDisabled(true); -- no quick combat !
...
function OnGameChangedVolumeLevel( iVolumeKnobID, fNewVolume )
	...
	-- format away any decimal junk by rounding down after padding a little bit.
	-- makes 0.8999 = 90%, 0.91 = 90%
	iNewVolume = math.floor((fNewVolume + 0.02) * 10) * 10;
	
	if Controls.MusicVolumeSlider:GetVoid1() == iVolumeKnobID then
		Controls.MusicVolumeSliderValue:SetText( Locale.ConvertTextKey("TXT_KEY_OPSCREEN_MUSIC_SLIDER", iNewVolume ) );
	elseif Controls.EffectsVolumeSlider:GetVoid1() == iVolumeKnobID then
		Controls.EffectsVolumeSliderValue:SetText( Locale.ConvertTextKey("TXT_KEY_OPSCREEN_SF_SLIDER", iNewVolume ) );
	elseif Controls.SpeechVolumeSlider:GetVoid1() == iVolumeKnobID then
		Controls.SpeechVolumeSliderValue:SetText( Locale.ConvertTextKey("TXT_KEY_OPSCREEN_SPEECH_SLIDER", iNewVolume ) );
	elseif Controls.AmbienceVolumeSlider:GetVoid1() == iVolumeKnobID then
		Controls.AmbienceVolumeSliderValue:SetText( Locale.ConvertTextKey("TXT_KEY_OPSCREEN_AMBIANCE_SLIDER", iNewVolume ) );
	end
end
...
function OnUIVolumeSliderValueChanged( fNewVolumeLevel, void1 )
    ...
    ----copied from OnGameChangedVolumeLevel()----
	-- format away any decimal junk by rounding down after padding a little bit.
	-- makes 0.8999 = 90%, 0.91 = 90%
	local iNewVolume = math.floor((fNewVolumeLevel + 0.02) * 10) * 10;
...
Controls.MusicVolumeSliderValue:SetText( Locale.ConvertTextKey("TXT_KEY_OPSCREEN_MUSIC_SLIDER", math.floor(GetVolumeKnobValue(iMusicVolumeKnobID)*100) ) );
Controls.EffectsVolumeSliderValue:SetText( Locale.ConvertTextKey("TXT_KEY_OPSCREEN_SF_SLIDER", math.floor(GetVolumeKnobValue(iSFXVolumeKnobID)*100) ) );
Controls.SpeechVolumeSliderValue:SetText( Locale.ConvertTextKey("TXT_KEY_OPSCREEN_SPEECH_SLIDER", math.floor(GetVolumeKnobValue(iSpeechVolumeKnobID)*100) ) );
Controls.AmbienceVolumeSliderValue:SetText( Locale.ConvertTextKey("TXT_KEY_OPSCREEN_AMBIANCE_SLIDER", math.floor(GetVolumeKnobValue(iAmbienceVolumeKnobID)*100) ) );
...
m_TutorialLevelText = {};
m_TutorialLevelText[ 0 ] = Locale.ConvertTextKey("TXT_KEY_OPSCREEN_TUTORIAL_LOW");
m_TutorialLevelText[ 1 ] = Locale.ConvertTextKey("TXT_KEY_OPSCREEN_TUTORIAL_MEDIUM");
m_TutorialLevelText[ 2 ] = Locale.ConvertTextKey("TXT_KEY_OPSCREEN_TUTORIAL_HIGH");
