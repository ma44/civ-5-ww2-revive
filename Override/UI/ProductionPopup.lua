...
function UpdateWindow( city )
    ...
    local numUnitButtons = 0;	
 	for unit in GameInfo.Units() do
 		...
 		if g_IsProductionMode then
			...
			-- R.E.D.
			if city:IsCanPurchase(false, unitID, -1, -1) then
 				local isDisabled = false;
     			-- test w/o visible (ie can train right now)
    ...
    Controls.ProduceRecruitsButton:SetHide( true );
    Controls.ProduceWarBondsButton:SetHide( true );
    if g_IsProductionMode then
 		for process in GameInfo.Processes() do
			...
			if city:CanMaintain( processID, 1 ) then
				...
				elseif process.Type == "PROCESS_RESEARCH" then
					...
					numOtherButtons = numOtherButtons + 1;
				-- Hardcoded Processes ?????????????
				-- R.E.D. WWII Processes
				elseif process.Type == "PROCESS_RECRUITING" then
					Controls.ProduceRecruitsButton:SetHide( false );
					Controls.ProduceRecruitsButton:SetVoid1( OrderTypes.ORDER_MAINTAIN ); 
					Controls.ProduceRecruitsButton:SetVoid2( processID );
					Controls.ProduceRecruitsButton:RegisterCallback( Mouse.eLClick, ProductionSelected );
					numOtherButtons = numOtherButtons + 1;
				elseif process.Type == "PROCESS_WAR_BONDS" then
					Controls.ProduceWarBondsButton:SetHide( false );
					Controls.ProduceWarBondsButton:SetVoid1( OrderTypes.ORDER_MAINTAIN ); 
					Controls.ProduceWarBondsButton:SetVoid2( processID );
					Controls.ProduceWarBondsButton:RegisterCallback( Mouse.eLClick, ProductionSelected );
					numOtherButtons = numOtherButtons + 1;
				end
			end
		end
