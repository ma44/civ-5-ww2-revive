...
function GetMaterielTTString()
	...
	if materielReinforcement then
		...
		if materielReinforcement.fromTrait		> 0 then strText = strText .. "[NEWLINE][ICON_BULLET] from trait (nation bonus) : "  .. materielReinforcement.fromTrait; end		
...
function GetOilTTString()
	...
	if resourceData and resourceData[playerID] then
		...
		local oilFromTrait = resourceData[playerID].OilFromTrait
		...
		if oilFromTrait			> 0 then strText = strText .. "[NEWLINE][ICON_BULLET] from trait : "  .. oilFromTrait; end		
