...
function OnUpdate( fDTime )
	...
	if (deferredDisplayTime > 0) then
		...
		deferredDisplayTime = deferredDisplayTime - fDTime;
		if (deferredDisplayTime <= 0) then	
...
function OnDisplay( type, team )
	...
	if( team == Game.GetActiveTeam() ) then
		...
		local REDVictoryType = savedData.GetValue("VictoryType")
		local victoryString = savedData.GetValue("VictoryString")
		if REDVictoryType ~= "default" then
			victoryType = REDVictoryType
			--text = GameInfo.Victories[victoryType].VictoryBackground
			Controls.EndGameText:SetText( victoryString )
		end
		-- R.E.D. WWII end ----------------------------------------------------------

		if(victoryType ~= nil and GameDefines.ALLOW_EXTENDED_PLAY == 1)then
		...
		Controls.BackButton:SetDisabled( true );
		print("VICTORY POPUP INFO");
		print(type);
		print(GameDefines.ALLOW_EXTENDED_PLAY);
		if (not Game:IsNetworkMultiPlayer() and player:IsAlive() and GameDefines.ALLOW_EXTENDED_PLAY == 1) then
		...
		-- R.E.D. WWII start --------------------------------------------------------
		if GetCivIDFromPlayerID(playerID) == HOTSEAT_CIV_TO_KILL then
			Controls.EndGameText:SetText( Locale.ConvertTextKey( "TXT_KEY_HOT_SEAT_FIX" ) )		
			Controls.BackButton:SetHide( true )	
			Controls.GameOverButton:SetHide( true )	
			Controls.DemographicsButton:SetDisabled( true )	
			Controls.RankingButton:SetDisabled( true )
			Controls.ReplayButton:SetDisabled( true )
		end
		-- R.E.D. WWII end ----------------------------------------------------------
