...
--
-- HACKED MOD STARTS (thanks to Whoward69)
--
local addinActions = {
--[[
  { Name = "Air Recon",
    Title = "Air Recon", -- or a TXT_KEY
    OrderPriority = 101, -- default is 200
    IconAtlas = "PROMOTION_ATLAS", -- 45 and 64 variations required
    PortraitIndex = 37,
    ToolTip = "Recon an area for hidden enemy units", -- or a TXT_KEY_ or a function
        ...
        then
            ...
            if(instance.UnitActionIcon ~= nil) then
				if action.SubType == ActionSubTypes.ACTIONSUBTYPE_PROMOTION then
					local thisPromotion = GameInfo.UnitPromotions[action.CommandData];
					IconHookup( thisPromotion.PortraitIndex, actionIconSize, thisPromotion.IconAtlas, instance.UnitActionIcon );
				else
					--local thisActionIcon = ActionIconLookup( action.Type );
...
function OnUnitActionClicked( action )
	if bOkayToProcess then
		if (GameInfoActions[action].SubType == ActionSubTypes.ACTIONSUBTYPE_PROMOTION) then
			...
			Game.ActiveUnitFlagNewPromotion(GameInfoActions[action].Type)
		end
    ...
    if (bDisabled) then
		...
		if (bBuild) then
			...
			if (pPlot:GetTeam() ~= unit:GetTeam()) then
				if (pImprovement and not pImprovement.OutsideBorders) then
