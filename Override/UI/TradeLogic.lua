...
function OnBack( iType )
    ...
    if( g_bPVPTrade or g_bTradeReview ) then
    	...
    	if (g_iDiploUIState == DiploUIStateTypes.DIPLO_UI_STATE_TRADE_AI_MAKES_DEMAND or
    		...
    		elseif (g_iDiploUIState == DiploUIStateTypes.DIPLO_UI_STATE_TRADE_AI_MAKES_REQUEST) then
    			Game.DoFromUIDiploEvent( FromUIDiploEventTypes.FROM_UI_DIPLO_EVENT_REQUEST_HUMAN_REFUSAL, g_iThem, 0, 0 );
    		end
...
function OnShowHide( isHide, bIsInit )
    ...
    if( not bIsInit ) then
        ...
        if( not isHide ) then		
    		...
    		--OpenAdvisorTutorialPopup(TutorialButtonPopupTypes.TUTORIALBUTTONPOPUP_DIPLOTRADE);
            
        -- Hiding screen
        else
    		UIManager:SetUICursor(oldCursor); -- make sure we retrun the cursor to the previous state
    		--CloseAdvisorTutorialPopup(TutorialButtonPopupTypes.TUTORIALBUTTONPOPUP_DIPLOTRADE);
...
function ResetDisplay()
    ...
    -- Are we not allowed to give Open Borders? (don't have tech, or are already providing it to them)
    ...
    if (not bResearchAgreementAllowed) then
		...
		if (Game.IsOption(GameOptionTypes.GAMEOPTION_NO_SCIENCE)) then
			strDisabledTT = " " .. Locale.ConvertTextKey( "TXT_KEY_DIPLO_RESCH_AGREEMENT_SCIENCE_OFF" );
		elseif (not g_pUsTeam:IsResearchAgreementTradingAllowed() and not g_pThemTeam:IsResearchAgreementTradingAllowed()) then
			strDisabledTT = " " .. Locale.ConvertTextKey( "TXT_KEY_DIPLO_RESCH_AGREEMENT_NO_TECH" );
		elseif (g_pUsTeam:IsHasResearchAgreement(g_iThemTeam)) then
			strDisabledTT = " " .. Locale.ConvertTextKey( "TXT_KEY_DIPLO_RESCH_AGREEMENT_EXISTS" );
		elseif (pUsTeamTechs:HasResearchedAllTechs() or pTheirTeamTechs:HasResearchedAllTechs()) then
			strDisabledTT = " " .. Locale.ConvertTextKey( "TXT_KEY_DIPLO_RESCH_AGREEMENT_ALL_TECHS_RESEARCHED" );
		else
			strDisabledTT = " " .. Locale.ConvertTextKey( "TXT_KEY_DIPLO_RESCH_AGREEMENT_NO_AGREEMENT" );
		end
...
-----------------------------------------------------------------------------------------------------------------------
-----------------------------------------------------------------------------------------------------------------------
function DisplayOtherPlayerItem( bFromUs, itemType, duration, OtherPlayer )
    if( TradeableItems.TRADE_ITEM_THIRD_PARTY_PEACE == itemType ) then
        --print( "Displaying Peace" );
    
        if( bFromUs ) then
            --print( "    from us" );
            g_OtherPlayersButtons[ OtherPlayer ].UsTablePeace.Button:SetHide( false );
        	Controls.UsTableMakePeaceStack:SetHide( false );
        	Controls.UsTableMakePeaceStack:CalculateSize();
        	Controls.UsTableMakePeaceStack:ReprocessAnchoring();
        else
            --print( "    from them" );
            g_OtherPlayersButtons[ OtherPlayer ].ThemTablePeace.Button:SetHide( false );
    ...
    elseif( TradeableItems.TRADE_ITEM_THIRD_PARTY_WAR == itemType ) then
        ...
        if( bFromUs ) then
            --print( "    from us" );
            g_OtherPlayersButtons[ OtherPlayer ].UsTableWar.Button:SetHide( false );
        	Controls.UsTableDeclareWarStack:SetHide( false );
        	Controls.UsTableDeclareWarStack:CalculateSize();
        	Controls.UsTableDeclareWarStack:ReprocessAnchoring();
        else
            --print( "    from them" );
            g_OtherPlayersButtons[ OtherPlayer ].ThemTableWar.Button:SetHide( false );
...
function AddPocketResource( row, isUs, stack )
    ...
    local iNumResource = pPlayer:GetNumResourceAvailable( row.ID, false );
    local strString = row.IconString .. " " .. Locale.ConvertTextKey(row.Description) .. " (" .. iNumResource .. ")";
...
function ChangeResourceAmount( string, control )
	...
	local pPlayer;
	if (bIsUs) then
		pPlayer = Players[ g_iUs ];
	else
		pPlayer = Players[ g_iThem ];
	end
 	...
 	for iLoopPlayer = 0, GameDefines.MAX_CIV_PLAYERS-1, 1 do
		...
		if( pLoopPlayer:IsAlive() ) then
    		...
    		if( g_iUsTeam ~= iLoopTeam and g_iThemTeam ~= iLoopTeam and
    		    ...
    		    g_OtherPlayersButtons[ iLoopPlayer ][ SubTableName ].Button:SetAlpha( 1 );
            else
            
    		    g_OtherPlayersButtons[ iLoopPlayer ][ SubTableName ].Button:SetDisabled( true );
    		    g_OtherPlayersButtons[ iLoopPlayer ][ SubTableName ].Button:SetAlpha( 0.5 );
			...
			-- Tooltip
			g_OtherPlayersButtons[ iLoopPlayer ][ SubTableName ].Button:SetToolTipString(strToolTip);
...
if( Game.IsOption( GameOptionTypes.GAMEOPTION_ALWAYS_WAR ) or
    ...
    for iLoopPlayer = 0, GameDefines.MAX_CIV_PLAYERS-1, 1 do
    	...
    	if( pLoopPlayer:IsEverAlive() ) then
			...
			print ("In trade logic for : "..szName)
