...
function AddBuildingButton( pCity, building )
	...
	if (pCity:IsHasBuilding(buildingID)) then
		...
		if (pCity:IsBuildingSellable(buildingID)) then
...
function UpdateThisQueuedItem(city, queuedItemNumber, queueLength)
    ...
    if (queuedOrderType == OrderTypes.ORDER_TRAIN) then
		...
		IconHookup( thisUnitInfo.PortraitIndex, 45, thisUnitInfo.IconAtlas, Controls[controlImage] );
...
function OnCityViewUpdate()
	...
	if (pCity ~= nil) then
		...
		OnCitySetDamage(pCity:GetDamage());
		...
		for pSpecialistInfo in GameInfo.Specialists() do
			local threshold = pCity:GetSpecialistUpgradeThreshold();			
...
-- On City Set Damage
-------------------------------------------------
function OnCitySetDamage(iDamage)
