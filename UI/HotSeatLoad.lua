...
function SetSelected( index )
    ...
    if( g_iSelected ~= -1 ) then
		...
		if(header ~= nil) then
			...
			Controls.StartEra:SetText(  Locale.ConvertTextKey("TXT_KEY_START_ERA", Locale.ConvertTextKey(GameInfo.Eras[header.StartEra].Description)));
			...
			for row in GameInfo.MapScripts{FileName = header.MapScript, SupportsSinglePlayer = 1, Hidden = 0} do
			...
			if(not bFound) then
				...
				if(mapData ~= nil) then
					...
					if(not Locale.IsNilOrWhitespace(name)) then
					...
					Controls.MapType:SetToolTipString(name);
				end
			end
