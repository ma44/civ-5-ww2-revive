...
---------------------------------------------------------
-- Requirements
UPDATE Units SET Requirements = (SELECT CivKey FROM UnitConfiguration WHERE 'UNIT_' || UnitConfiguration.UnitKey = Units.Type AND UnitConfiguration.CivKey NOT NULL)
			WHERE EXISTS	 (SELECT CivKey FROM UnitConfiguration WHERE 'UNIT_' || UnitConfiguration.UnitKey = Units.Type AND UnitConfiguration.CivKey NOT NULL);
...
INSERT OR REPLACE INTO Projects (Type, Description, Help, Civilopedia, Strategy, MaxGlobalInstances, MaxTeamInstances, Cost, IconAtlas, PortraitIndex)
	SELECT 'PROJECT_' || UnitKey, 'TXT_KEY_PROJECT_' || UnitKey, 'TXT_KEY_PROJECT_' || UnitKey || '_DESC', Units.Civilopedia, Units.Strategy, -1, 1, ROUND(Units.Cost*0.5), Units.IconAtlas, Units.PortraitIndex
