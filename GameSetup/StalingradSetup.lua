...
scenarioMap = Modding.GetEvaluatedFilePath(myModId, myModVersion, "Stalingrad.Civ5Map");
...
Controls.StartButton:RegisterCallback(Mouse.eLClick, function()
	...
	elseif (not g_bDataLoaded ) then
		...
		print("-------------------------------------")
		modUserData.SetValue("RedLoading", "Stalingrad")
...
function RefreshCheckBoxGameOptions()
	...
	g_REDCheckboxOptions = {

		{
			...
			Type = "AIoverride",
			Name = "AI override",
			ToolTip = "Attempt to give some simulated neurones to the AI, especially for air and naval units",
			Disabled = false,
			GameOption = true,
			Checked = true,
			SortPriority = 3,
...
function Initialize()
	...
	print("-------------------------------------")
	print ("R.E.D WW2 - Stalingrad setup screen")
	...
	Controls.RedWW2Logo:SetTextureAndResize( "Stalingrad.dds" )

	local handicap = modUserData.GetValue ("Handicap")
	print("Handicap is " .. tostring(handicap))
	print("-------------------------------------")
	if not handicap then
		...
		PreGame.SetHandicap( 0, g_CurrentDifficulty )
		modUserData.SetValue ("Handicap", g_CurrentDifficulty)
	else		
		PreGame.SetHandicap( 0, handicap )
		g_CurrentDifficulty = handicap
	end
...
function SetPlayerIndex(playerIndex)
	...
	if(playerIndex ~= nil) then		
		...
		-- Sets Dawn of Man Quote
		Controls.Quote:LocalizeAndSetText("TXT_KEY_RED_STALINGRAD_BRIEFING");		
    end
