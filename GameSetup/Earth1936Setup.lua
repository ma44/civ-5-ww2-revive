...
ScenarioCivilizations = {
	...
	[2] = "CIVILIZATION_JAPAN",
	[3] = "CIVILIZATION_ROME",
	[4] = "CIVILIZATION_RUSSIA",
	[5] = "CIVILIZATION_FRANCE",
	[6] = "CIVILIZATION_ENGLAND",
	[7] = "CIVILIZATION_CHINA",
	[8] = "CIVILIZATION_AMERICA",
}
...
myModId = "1fa4b768-ae75-4260-b727-6a2becc6530e";
myModVersion = Modding.GetActivatedModVersion(myModId);
scenarioMap = Modding.GetEvaluatedFilePath(myModId, myModVersion, "1937_Earth.Civ5Map");
...
Controls.StartButton:RegisterCallback(Mouse.eLClick, function()
	...
	elseif (not g_bDataLoaded ) then
		...
		modUserData.SetValue("RedLoading", "Earth1936")
...
function RefreshCheckBoxGameOptions()
	...
	g_REDCheckboxOptions = {

		{
			...
			ToolTip = "Allow declarations of war outside the historic calendar (After 3 Turns)",
			Disabled = false,
			GameOption = true,
			SortPriority = 1,
		},	{
			...
			Type = "PlayEpicGame",
			Name = "Play Epic Game",
			ToolTip = "Change the calendar from a normal 10 day turnaround to a 5 day turnaround.",
			Disabled = false,
			GameOption = true,
			SortPriority = 1,
		},		{
			Type = "PlayWithSeason",
			Name = "Play With Seasons",
			ToolTip = "Terrain will be changed in wintertime (Game must saved and reloaded to see effects on map).",
			Disabled = false,
			GameOption = true,
			SortPriority = 1,
		},
...
function Initialize()
	...
	print("-------------------------------------")
	print ("R.E.D WW2 - Earth 1936 setup screen")
	local gameVersion = UI.GetVersionInfo()
	print ("game version : " .. gameVersion )
	print("-------------------------------------")

	local RedWW2ModID = "1fa4b768-ae75-4260-b727-6a2becc6530e"
	local RedWW2DataFilesModID = "2ac737a3-00a3-4d33-9b88-714266967b12"
		...
		if modInfo.Enabled then
			if (modInfo.Name) then
				...
				if ( modInfo.ID ~= RedWW2ModID and modInfo.ID ~= RedWW2DataFilesModID ) then 
					--g_bCanLoad = false
				elseif ( modInfo.ID == RedWW2DataFilesModID and modInfo.Version >= 1) then
					g_bDataLoaded = true
				end
			end
		end
	...
	Controls.RedWW2Logo:SetTextureAndResize( "Earth_1936.dds" )
...
function SetPlayerIndex(playerIndex)
	...
	if(playerIndex ~= nil) then		
		...
		-- Sets Dawn of Man Quote
		Controls.Quote:LocalizeAndSetText("TXT_KEY_RED_EARTH1936_BRIEFING");		
    end
