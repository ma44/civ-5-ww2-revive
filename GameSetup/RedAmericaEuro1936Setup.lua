...
ScenarioCivilizations = {
	...
	[2] = "CIVILIZATION_AMERICA",
	[3] = "CIVILIZATION_ROME",
	[4] = "CIVILIZATION_ENGLAND",
	[5] = "CIVILIZATION_RUSSIA",
	[6] = "CIVILIZATION_GERMANY",
}
...
scenarioMap = Modding.GetEvaluatedFilePath(myModId, myModVersion, "Europe_1937.Civ5Map");
...
Controls.StartButton:RegisterCallback(Mouse.eLClick, function()
	...
	elseif (not g_bDataLoaded ) then
		...
		modUserData.SetValue("RedLoading", "AmericaEuro1936")
...
function RefreshCheckBoxGameOptions()
	...
	g_REDCheckboxOptions = {

		{
			...
			ToolTip = "Check this to enable hardcore mode. When hardcore mode is enabled, the AI will never stop building units and will suicide attack.",
			Disabled = false,
			GameOption = true,
			Checked = false,
			SortPriority = 3,
	...
	for i, option in ipairs(g_REDCheckboxOptions) do
		...
		if option.Checked ~= nil then -- there is an override for this scenario
...
function Initialize()
	...
	print("-------------------------------------")
	print ("R.E.D WW2 - North America and Europe setup screen")
	...
	Controls.RedWW2Logo3:SetTextureAndResize( "1936MainMenu.dds" )
...
function SetPlayerIndex(playerIndex)
	...
	if(playerIndex ~= nil) then		
		...
		-- Sets Dawn of Man Quote
		Controls.Quote:LocalizeAndSetText("TXT_KEY_RED_AMERICAEURO1936_BRIEFING");		
    end
